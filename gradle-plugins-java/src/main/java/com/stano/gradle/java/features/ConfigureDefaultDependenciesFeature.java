...
import com.stano.gradle.java.JavaExtension;
  ...
  private boolean hasMapStructDependency(Project project) {
    try {
      return project
          ...
          .anyMatch(
              a ->
                  "org.mapstruct".equals(a.getModuleVersion().getId().getModule().getGroup())
                      && "mapstruct".equals(a.getModuleVersion().getId().getModule().getName()));
    } catch (Exception e) {
      return false;
    }
  }
