...
## Adding a New Plugin Submodule

When creating a new `gradle-plugins-<name>` submodule that defines a Gradle plugin, all of the following are required — missing any of them causes consumers to hit `Plugin [id: '...'] was not found in any of the following sources` or similar resolution failures:

1. Declare the plugin in the submodule's `build.gradle.kts` via the `gradlePlugin { plugins { create("<name>") { id = "com.stano.<name>"; implementationClass = "..." } } }` DSL (see any existing `gradle-plugins-*/build.gradle.kts` for the pattern). This is what generates the `META-INF/gradle-plugins/<id>.properties` marker file and the plugin marker Maven publication (`<id>.gradle.plugin`) that `plugins { id("com.stano.<name>") }` resolves against. **Never** hand-write `META-INF/gradle-plugins/*.properties` — it's a generated build output (via the `pluginDescriptors` task), just like the plugin marker publication is generated by `java-gradle-plugin` + `maven-publish`.
2. Add `"com.stano.<name>"` to the self-pinned plugin id list in `gradle-plugins-settings/src/main/java/com/stano/gradle/settings/SettingsPlugin.java` (the `List.of(...)` in `apply()`). **This step is the one most easily forgotten.** Skipping it means a consumer applying `id("com.stano.<name>")` with no version gets a "Plugin not found" error, because `com.stano.settings` has nothing to pin a version to. Sanity check: every plugin ID in the Plugin Map table below should also appear in that `List.of(...)`.
3. Add an `implementation` constraint for the new submodule's JAR to `gradle-plugins-bom/build.gradle.kts`.
4. Add a row for the plugin to the Plugin Map table below, and a row to the Package Naming table if it introduces a new package.
5. Ensure the submodule is `include(...)`d (active, not commented out) in `settings.gradle.kts`.
