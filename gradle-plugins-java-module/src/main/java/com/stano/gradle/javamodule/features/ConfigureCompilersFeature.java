...
public class ConfigureCompilersFeature implements PluginFeature {
  ...
  public void apply(Project project) {
    ...
    if (!currentJavaVersion.isCompatibleWith(JavaVersion.toVersion(projectJavaVersion))) {
      throw new GradleException(String.format("The current java version %s is not compatible with the project java version %s",
                                              currentJavaVersion,
                                              JavaVersion.toVersion(projectJavaVersion)));
    }

    project.getExtensions().configure(org.gradle.api.plugins.JavaPluginExtension.class, javaPluginExtension -> {
      javaPluginExtension.getToolchain().getLanguageVersion().set(JavaLanguageVersion.of(projectJavaVersion.getMajorVersion()));
    });
