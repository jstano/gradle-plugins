...
class DockerPlugin implements Plugin<Project> {
   ...
   @Inject
   DockerPlugin(ObjectFactory objectFactory) {
   ...
   void apply(Project project) {
      ...
      if (project.tasks.findByName("copyWar")) {
         prepare.dependsOn(project.tasks.findByName("copyWar"))
      }
      ...
      Task copyDockerImageUrl = project.tasks.create('dockerImageUrl', {
         group = 'Docker'
         description = 'Copies Docker image url to a file.'
         doLast {
            def content = project.getExtensions().getByType(DockerExtension.class).getName();
            def fileName = "${project.buildDir}/docker-image-url.txt"
      ...
      PublishArtifact dockerArtifact = new ArchivePublishArtifact(DefaultTaskDependencyFactory.withNoAssociatedProject(), dockerfileZip)
   ...
   private List<String> buildCommandLine(DockerExtension ext) {
      ...
      if (ext.buildx) {
         ...
         buildCommandLine.addAll('--platform', String.join(',', platform))
