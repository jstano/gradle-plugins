...
class DockerComposePlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        project.extensions.create('dockerCompose', com.palantir.gradle.docker.DockerComposeExtension, project)
        ...
        project.tasks.create('generateDockerCompose', com.palantir.gradle.docker.GenerateDockerCompose, {
            it.configuration = dockerConfiguration
        })

        project.tasks.create('dockerComposeUp', com.palantir.gradle.docker.DockerComposeUp, {
            it.configuration = dockerConfiguration
        })

        project.tasks.create('dockerComposeDown', com.palantir.gradle.docker.DockerComposeDown, {
            it.configuration = dockerConfiguration
        })
    }
}
