...
public class DockerRunPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        Exec dockerRunStatus = project.getTasks().create("dockerRunStatus", Exec.class, task -> {
            task.setGroup("Docker Run");
            task.setDescription("Checks the run status of the container");
        });

        Exec dockerRun = project.getTasks().create("dockerRun", Exec.class, task -> {
            task.setGroup("Docker Run");
            task.setDescription("Runs the specified container with port mappings");
        });

        Exec dockerStop = project.getTasks().create("dockerStop", Exec.class, task -> {
            task.setGroup("Docker Run");
            task.setDescription("Stops the named container if it is running");
            task.setIgnoreExitValue(true);
        });

        Exec dockerRemoveContainer = project.getTasks().create("dockerRemoveContainer", Exec.class, task -> {
            task.setGroup("Docker Run");
            task.setDescription("Removes the persistent container associated with the Docker Run tasks");
            task.setIgnoreExitValue(true);
        });

        Exec dockerNetworkModeStatus = project.getTasks().create("dockerNetworkModeStatus", Exec.class, task -> {
            task.setGroup("Docker Run");
        ...
        project.afterEvaluate(p -> {
            ...
            dockerRunStatus.commandLine("docker", "inspect", "--format={{.State.Running}}", ext.getName());
            dockerRunStatus.doLast(task -> {
            ...
            dockerNetworkModeStatus.commandLine("docker", "inspect", "--format={{.HostConfig.NetworkMode}}", ext.getName());
            dockerNetworkModeStatus.doLast(task -> {
                String networkMode = dockerNetworkModeStatus.getStandardOutput().toString().trim();
                if ("default".equals(networkMode)) {
                    System.out.println("Docker container '" + ext.getName() + "' has default network configuration (bridge).");
                } else if (networkMode.equals(ext.getNetwork())) {
                    System.out.println("Docker container '" + ext.getName() + "' is configured to run with '" + ext.getNetwork() + "' network mode.");
                } else {
                    System.out.println("Docker container '" + ext.getName() + "' runs with '" + networkMode + "' network mode instead of the configured '" + ext.getNetwork() + "'.");
                }
            });
            ...
            for (Map.Entry<Object, String> volume : ext.getVolumes().entrySet()) {
                ...
                if (!localFile.exists()) {
                    System.err.println("ERROR: Local folder " + localFile + " doesn't exist. Mounted volume will not be visible to container");
